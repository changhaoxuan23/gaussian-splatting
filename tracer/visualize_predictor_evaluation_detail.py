...
# collect ranges
_maximums = dict.fromkeys(_entries)
for path in path_list:
  data = torch.load(path)
  for entry in _entries:
    maximum = data[entry].abs().max()
    _maximums[entry] = maximum if _maximums[entry] is None else max(_maximums[entry], maximum)

_ranges = {}
for entry in _entries:
  _space = torch.logspace(-35, torch.log(_maximums[entry]), _data_columns // 2 + 1, base=torch.e)
  _space[0] = 0
  space = torch.cat((-_space.flip(dims=(0,)), _space[1:]))
  _ranges[entry] = space
...
for entry, image in images.items():
  ...
  _block_size = f"{(_ranges[entry][1] / _ranges[entry][0]).item()}x"
  draw.text(
    xy=(
      _spacings[3],
      _spacings[0] + _pixel_size * len(path_list) + _border_width + _anchor_length + _annotation_space,
    ),
    text=f"{_ranges[entry][0].item()} (1 block = {_block_size})",
    fill="white",
    font=font,
    anchor="lt",
  )
