...
_ranges = {
  "opacity": torch.linspace(_ranges["opacity"][0], _ranges["opacity"][1], _data_columns + 1),
  "volume": torch.logspace(
    torch.log(_ranges["volume"][0]),
    torch.log(_ranges["volume"][1]),
    _data_columns + 1,
    base=torch.e,
  ),
}
...
for entry, image in images.items():
  ...
  _block_size = (
    f"+-{(_ranges[entry][1] - _ranges[entry][0]).item()}"
    if entry == "opacity"
    else f"*/{(_ranges[entry][1] / _ranges[entry][0]).item()}"
  )
  draw.text(
    xy=(
      _spacings[3],
